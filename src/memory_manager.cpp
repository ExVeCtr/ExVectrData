...
namespace VCTR
{
    ...
    namespace Data
    {
        ...
        Memory_Manager::Memory_Manager(Memory_Interface &memory) : memory(memory)
        {
        }
        ...
        void Memory_Manager::deallocateItem(uint32_t key)
        {
            ...
            if (!memory.readMem(allocatedItems))
                return;
            ...
            if (lastIndex == 0)
            { // Item to be removed is the first item in the memory.
        ...
        void Memory_Manager::clearMemory()
        {
            ...
            if (version != VERSION_)
            { // Memory is a different version. We do not want to simply clear the memory, as this will delete all data.
        ...
        void Memory_Manager::findItem(uint32_t &lastIndex, uint32_t &indexFound, uint32_t &nextIndex, uint32_t key, uint32_t numBytes)
        {
            ...
            if (!memory.readMem(memoryHeader))
                return; // Read the header byte.
            if (!memory.readMem(version))
                return; // Read the version byte.
            if (!memory.readMem(allocatedItems))
                return;
            if (!memory.readMem(firstItemIndex))
                return; // Read the index to the first item in the memory.
            ...
            while (indexFound < memory.size())
            {
                ...
                if (!memory.readMem(header))
                    break; // Item is not valid. This should be a free space.
                if (!memory.readMem(nextIndex))
                    break; // Failure to read memory.
                if (!memory.readMem(keyFound))
                    break; // Failure to read memory.
                if (!memory.readMem(size))
                    break; // Failure to read memory.

                // LOG_MSG("Searching for item. Index: %d, Header: %x, Next Index: %d, Key: %d, Size: %d\n", indexFound, header, nextIndex, keyFound, size);

                if (header != 0xEFA6 && indexFound == 20)
                    return; // Item is not valid. Something is wrong

                // Per item checking
                if (key != 0 && key == keyFound && (size == numBytes || numBytes == 0))
                    return; // Item we are searching for.

                // Cases for when the we have reached the end of the memory.
                if (nextIndex == 0 && key != 0)
                    break; // That was the final item. If we are looking for a key, then it was not found.
                if (nextIndex == 0 && key == 0)
                {                                        // If we are looking for the final item, then we found it. Return the index.
                ...
                if (itemsFound >= allocatedItems)
                    break; // We have traversed all items in the memory. Something is wrong...
        ...
        uint32_t Memory_Manager::allocateSpace(uint32_t size, uint32_t key)
        {
            ...
            if (indexFound == 0)
            {
                LOG_MSG("No free space found in the memory.\n");
                return 0; // No free space found in the memory.
            }
            ...
            if (key == 0)
                key = indexFound; // Set the key to the index found.
            ...
            if (lastIndex == 0)
            { // If this is the first item in the memory, then we need to set the index to the first item in the memory.
        ...
        uint8_t Memory_Manager::getMemoryVersion()
        {
            ...
            memory.accessIndex(0); // Set the access index to the beginning of the memory.
            if (!memory.readMem(header))
                return 0; // Failure to read memory.
            if (!memory.readMem(version))
                return 0; // Failure to read memory.
            if (header != 0xABFA)
                return 0;   // Memory is not initialized.
