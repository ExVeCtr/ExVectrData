...
namespace VCTR
{
    ...
    namespace Data
    {
        ...
        {
            if (bufferIndex + numBytes > memorySize_)
                numBytes = memorySize_ - bufferIndex; // Out of bounds. Reduce the number of bytes to be read.
            if (index + numBytes > memorySize_)
                numBytes = memorySize_ - index; // Out of bounds. Reduce the number of bytes to be read.
            if (readBus(bufferPtr, numBytes, index))
            {
                LOG_MSG("failed to read data from bus!\n");
                return 0;
            }
            return numBytes;
        }
        ...
        {
            if (bufferIndex + numBytes > memorySize_)
                numBytes = memorySize_ - bufferIndex; // Out of bounds. Reduce the number of bytes to be written.
            if (index + numBytes > memorySize_)
                numBytes = memorySize_ - index; // Out of bounds. Reduce the number of bytes to be written.
            if (writeBus(bufferPtr, numBytes, index))
            {
                LOG_MSG("failed to write data to bus!\n");
                return 0;
            }
            return numBytes;
        }
        ...
        size_t Memory_AT24CX::transferFrom(Memory_Interface &memory, size_t numBytes, size_t toIndex, size_t fromIndex)
        {
            if (toIndex + numBytes > memorySize_)
                numBytes = memorySize_ - toIndex; // Out of bounds. Reduce the number of bytes to be written.
            if (fromIndex + numBytes > memory.size())
                numBytes = memory.size() - fromIndex; // Out of bounds. Reduce the number of bytes to be written.
            uint8_t buffer_[numBytes];
            if (memory.readMem(buffer_, numBytes, fromIndex) != numBytes)
            {
                LOG_MSG("failed to read data from given memory!\n");
                return 0;
            }
        ...
        bool Memory_AT24CX::writeBus(const uint8_t *bufferPtr, size_t numBytes, size_t address)
        {
            ...
            {
                LOG_MSG("failed to write address to bus!\n");
